...
const APP_DISPLAY_NAME = isDevelopment ? "Pericode (PP Edition)" : "Pericode (PP Edition)";
const APP_USER_MODEL_ID = "com.t3tools.t3code";
const USER_DATA_DIR_NAME = isDevelopment ? "t3code-dev" : "t3code";
const LEGACY_USER_DATA_DIR_NAME = isDevelopment
  ? "Pericode (PP Edition) (Dev)"
  : "Pericode (PP Edition) (Alpha)";
...
function handleFatalStartupError(stage: string, error: unknown): void {
  ...
  if (!isQuitting) {
    isQuitting = true;
    dialog.showErrorBox(
      "Pericode (PP Edition) failed to start",
      `Stage: ${stage}\n${message}${detail}`,
    );
  }
  stopBackend();
...
async function checkForUpdatesFromMenu(): Promise<void> {
  ...
  if (updateState.status === "up-to-date") {
    void dialog.showMessageBox({
      ...
      message: `Pericode (PP Edition) ${updateState.currentVersion} is currently the newest version available.`,
      buttons: ["OK"],
    });
 ...
 * parentheses (e.g. `~/.config/Pericode (PP Edition) (Alpha)` on Linux). This is
