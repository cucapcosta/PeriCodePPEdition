...
const makeServerRuntimeProgram = (input: CliInput) =>
  Effect.gen(function* () {
    ...
    yield* Effect.logInfo("Pericode (PP Edition) running", {
      ...safeConfig,
...
const t3HomeFlag = Flag.string("home-dir").pipe(
  Flag.withDescription(
    "Base directory for all Pericode (PP Edition) data (equivalent to T3CODE_HOME).",
  ),
  Flag.optional,
);
...
export const t3Cli = Command.make("t3", {
  ...
  Command.withDescription("Run the Pericode (PP Edition) server."),
